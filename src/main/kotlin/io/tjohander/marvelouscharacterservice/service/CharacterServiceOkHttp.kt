...
class CharacterServiceOkHttp(
    ...
    override fun getCharacterStartsWith(startsWithString: String): Character? =
        characterRepository.findCharacterByNameIs(startsWithString) ?: getCharacterFromMarvelByName(startsWithString)

    override fun getCharactersFromMarvelPaged(limit: Int, offset: Int): List<Character>? {
        ...
        val url = HttpUrl.Builder()
            ...
            .addQueryParameter(MarvelQueryParam.LIMIT.value, limit.toString())
            .addQueryParameter(MarvelQueryParam.OFFSET.value, offset.toString())
        ...
        client.newCall(request).execute().use { response ->
            ...
            val characters: List<Character>? = jsonAdapter.fromJson(response.body!!.string())?.data?.results?.let { results ->
                characterRepository.saveAll(results)
            }
            return characters
        }
    }

    private fun getCharacterFromMarvelByName(startsWithString: String): Character? {
        val auth = MarvelAuthGenerator.buildAuthString(
            Instant.now(),
            marvelApiPublicKey,
            marvelApiPrivateKey)
